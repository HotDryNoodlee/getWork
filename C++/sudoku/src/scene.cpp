...
void CScene::printUnderline(int line_no) const {
  if ((line_no + 1)%3 == 0) {
    ...
    for (int colunm = 0; colunm < 9; ++colunm) {
        ...
        std::cout << Color::Modifier(Color::FG_LIGHT_BLUE) << CORNER << LINE << third_symbol << LINE;
    }
    std::cout << CORNER << Color::Modifier(Color::RESET) << std::endl;
  }
  else {
    auto is_curline = (_cur_point.y == line_no);
    for (int colunm = 0; colunm < 9; ++colunm) {
        if ((colunm)%3 == 0) {
            auto third_symbol = (is_curline && _cur_point.x == colunm) ? ARROW : LINE;
            std::cout << Color::Modifier(Color::FG_LIGHT_BLUE) << CORNER << Color::Modifier(Color::RESET) <<  LINE << third_symbol << LINE;
        } else {
            auto third_symbol = (is_curline && _cur_point.x == colunm) ? ARROW : LINE;
            std::cout<< CORNER << LINE << third_symbol << LINE;
        }
    }
    std::cout << Color::Modifier(Color::FG_LIGHT_BLUE) << CORNER << Color::Modifier(Color::RESET) << std::endl;
  }
}
